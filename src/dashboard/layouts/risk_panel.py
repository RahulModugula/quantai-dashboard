    ...
    return html.Div([
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader([
                        html.H5("Backtest Configuration", className="mb-0 d-inline"),
                        dbc.Badge("Educational Only", color="warning", className="ms-2"),
                    ]),
                    dbc.CardBody([
                        dbc.Row([
                            dbc.Col([
                                ...
                                dcc.Dropdown(
                                    id="backtest-ticker",
                                    options=[{"label": t, "value": t} for t in ["AAPL", "MSFT", "GOOGL", "AMZN", "SPY"]],
                                    value="AAPL",
                                    clearable=False,
                                ),
                            ], width=3),
                            dbc.Col([
                                ...
                                dbc.Input(id="backtest-capital", value=100000, type="number"),
                            ], width=3),
                            dbc.Col([
                                html.Label("Buy Threshold"),
                                dbc.Input(id="buy-threshold", value=0.6, type="number", step=0.05),
                            ], width=3),
                            dbc.Col([
                                html.Label(" "),
                                dbc.Button("Run Backtest", id="run-backtest-btn", color="primary", className="w-100 d-block mt-4"),
                            ], width=3),
                        ]),
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardBody([dcc.Graph(id="backtest-equity-chart", style={"height": "300px"})]),
                ])
            ], width=8),
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader("Drawdown"),
                    dbc.CardBody([dcc.Graph(id="backtest-drawdown-chart", style={"height": "300px"})]),
                ])
            ], width=4),
        ]),
    ])
