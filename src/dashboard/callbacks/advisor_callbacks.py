...
def register_advisor_callbacks(app):
    ...
    def get_advice(n_clicks, age, horizon, income, loss_tol, emergency, debt):
        try:
            resp = httpx.post(f"{BASE_URL}/api/advisor/risk-profile", json={
                "age": age or 30,
            ...
            rp = RP(score=profile["score"], category=profile["category"], description=profile["description"])
            ...
            rec_rows = []
            for r in recs[:6]:
                color = "success" if r["action"] == "increase" else "danger" if r["action"] == "reduce" else "secondary"
                rec_rows.append(html.Tr([
                    html.Td(r["asset_class"]),
                    html.Td(f"{r['current_pct']}%"),
                    html.Td(f"{r['target_pct']}%"),
                    html.Td(dbc.Badge(r["action"], color=color, className="text-uppercase")),
                ]))

            rec_table = dbc.Table([
                html.Thead(html.Tr([html.Th("Asset"), html.Th("Current"), html.Th("Target"), html.Th("Action")])),
                html.Tbody(rec_rows),
